#include <stdio.h>
#include <SoftwareSerial.h> // included in arduino extension for VScode
...
void loop() {
  // // specify SoftwareSerial object to "listen" (Receive on RX pin)
  ble.listen();

  // // verifying our device is listening
  // while (!ble.isListening()) {
  //   delay(100);
  //   Serial.println("I am not listening!");
  // }
  // if (ble.isListening()) {
  //   Serial.println("I am listening! Yay!");
  // }

  while (ble.available() > 0) {   // if HM10 sends something then read
    appData = (ble.read() - 48);
    ...
    if (appData == 1) {
      ...
      while (stepper1.currentPosition != stepper1.stepsPerRev) { 
        appData = (ble.read() - 48); // idk why you send a "1" via the BLE app console and it receives "48"
        if (appData == 0) break;
        ...
        // telling the RFSoC board that the motor has stepped to position
        // (and that it should start taking a measurement)
        ble.write(READY);
